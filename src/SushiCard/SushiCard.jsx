...
export const SushiCard = ({ sushi }) => {
  const { weight, name, cost, url } = sushi;
  ...
  return (
    ...
    >
      <img src={url} alt={name} className='img-fluid rounded mx-auto d-block mt-2 card-image-top card-img'/>
      <div className="card-body pb-0">
        <p className='text-secondary-emphasis fs-2 fst-italic'>{name}</p>
        <p className='text-secondary-emphasis fs-5'>{`Вага: ${weight}г`}</p>
        <p className='text-secondary-emphasis fs-5 mb-2'>{`Ціна: ${cost}`}</p>
      </div>
    </a>
  );
};